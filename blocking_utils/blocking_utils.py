...
def compute_similarity(row1, row2):
    # Initialize similarity score and total weight
    ...
    if row1["is_company"] and row2["is_company"]:
    ...
    if row1["parsed_name"] and row2["parsed_name"]:
        name_similarity = fuzz.WRatio(row1["parsed_name"], row2["parsed_name"]) / 100
    ...
    phonetic_matches = 0
    phonetic_total = 0
    phonetic_encodings = ["name_soundex", "name_metaphone", "name_nysiis"]
    for encoding in phonetic_encodings:
        if row1.get(encoding) and row2.get(encoding):
            phonetic_total += 1
    ...
    if row1["surname"] and row2["surname"]:
        surname_similarity = fuzz.WRatio(row1["surname"], row2["surname"]) / 100
    ...
    surname_phonetic_encodings = ["surname_soundex", "surname_metaphone", "surname_nysiis"]
    for encoding in surname_phonetic_encodings:
        if row1.get(encoding) and row2.get(encoding):
    ...
    if row1["given_name"] and row2["given_name"]:
        given_name_similarity = fuzz.WRatio(row1["given_name"], row2["given_name"]) / 100
        similarity_score += given_name_similarity * weights["given_name"]
        total_weight += weights["given_name"]

    # 7. Compare 'surname_length'
    if row1["surname_length"] and row2["surname_length"]:
    ...
    if row1["party_iban"] and row2["party_iban"]:
    ...
    if row1["party_phone"] and row2["party_phone"]:
        phone_similarity = fuzz.WRatio(row1["party_phone"], row2["party_phone"]) / 100
